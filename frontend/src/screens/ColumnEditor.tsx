...
export function ColumnEditor({
  ...
  /** La columna que se renombra o se comenta. Vacío al agregar. */
  columna?: DetailColumn;
  /** Si el motor guarda comentarios. Con SQLite el campo no se muestra: no
   *  existen, y ofrecerlo haría fallar el apply entero. */
  ...
  const admiteModificador = elegido
    ? elegido.acceptsModifier
    : tipo.trim() !== "";
  const tipoCompleto =
    tipo.trim() +
    (modificador.trim() ? `(${modificador.trim()})` : "") +
    (arreglo ? "[]" : "");
  ...
  // Al comentar no hay nada obligatorio: un comentario vacío es «sacale el
  // comentario», que es una operación legítima.
  const puede =
    comentar ||
    (!nombreVacio && (renombrar || tipo.trim() !== "") && !faltaDefault);
  ...
  return (
    <Dialog
      ...
      title={
        comentar
          ? `Comentario de ${columna?.name}`
          : renombrar
            ? `Renombrar ${columna?.name}`
            : `Agregar una columna a ${tabla}`
      }
      onClose={onCerrar}
      footer={
        <>
          ...
          <Button
            ...
            {...(puede
              ? {}
              : { title: razonDeBloqueo(nombreVacio, faltaDefault, tipo) })}
            onClick={() =>
              onGuardar({
                name: nombre.trim(),
        ...
        {comentar ? (
          <>
            ...
            <p className={styles.nota}>
              Vacío le saca el comentario. Es lo único que cambia: los datos y
              el tipo quedan como están.
            </p>
          </>
        ) : (
          <>
            ...
            {renombrar ? (
              <p className={styles.nota}>
                El nombre cambia en la base. Todo lo que la nombre —consultas
                guardadas, vistas, código— deja de encontrarla.
              </p>
            ) : (
              <>
                <Field label="Tipo">
                ...
                <p className={styles.nota}>
                  Es una <strong>expresión</strong>, no un valor:{" "}
                  <code>now()</code> llama a la función y{" "}
                  <code>&apos;now()&apos;</code> guarda ese texto. Las cadenas
                  van entre comillas simples.
                </p>

                {faltaDefault ? (
                  <p className={styles.error}>
                    Una columna que no admite nulos necesita un valor por
                    defecto: las filas que ya están tienen que tener qué poner.
                  </p>
                ) : null}

                {defaultSospechoso ? (
                  <p className={styles.error}>
                    <code>{porDefecto.trim()}</code> se va a leer como el nombre
                    de otra columna, y un valor por defecto{" "}
                    <strong>no puede nombrar columnas</strong>: se calcula sin
                    ninguna fila a la vista. Si querías el texto, va entre
                    comillas simples; si el valor tiene que salir de otra
                    columna, lo que hace falta es una columna generada, no un
                    default.
                  </p>
                ) : null}
 ...
 */
function razonDeBloqueo(
  nombreVacio: boolean,
  faltaDefault: boolean,
  tipo: string,
): string {
  if (nombreVacio) return "Falta el nombre de la columna";
  if (tipo.trim() === "") return "Falta el tipo";
  if (faltaDefault)
    return "Una columna que no admite nulos necesita un valor por defecto";
  return "";
}

/** Qué se espera adentro de los paréntesis, según el tipo. */
function placeholderDeModificador(tipo: string): string {
  if (tipo.startsWith("numeric") || tipo.startsWith("decimal"))
    return "10,2  ·  precisión, escala";
  if (tipo.startsWith("character") || tipo.startsWith("bit"))
    return "255  ·  largo";
  if (tipo.startsWith("time") || tipo.startsWith("interval"))
    return "3  ·  dígitos de segundo";
